...
from backend.app.services import cf_service
from backend.app.services.cf_service import train_cf_models
...
def main() -> None:
    ...
    with mlflow.start_run(run_name="train_cf"):
        metrics = train_cf_models(Path(settings.processed_data_dir), Path(settings.artifact_dir))
        eval_metrics = _evaluate_cf(Path(settings.processed_data_dir), Path(settings.artifact_dir))
...
def _evaluate_cf(processed_dir: Path, artifact_dir: Path, sample_users: int = 50, k: int = 10) -> dict:
    ...
    mappings_path = artifact_dir / "mappings" / "cf_mappings.json"
    mappings = json.load(mappings_path.open("r", encoding="utf-8"))
    ...
    for user_id in sampled:
        ...
        scores = cf_service.score_candidates_for_user(
            ratings,
            item_similarity,
            user_similarity,
            movie_id_to_idx,
